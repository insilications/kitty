...
def parse(lines: Iterable[str]) -> Iterator[OpenAction]:
    ...
    with to_cmdline_implementation.filter_env_vars(
        'URL', 'FILE_PATH', 'FILE', 'FRAGMENT', 'URL_PATH', 'NETLOC', 'MOUSE_EVENT_MOD', EDITOR=shlex.join(get_editor()), SHELL=resolved_shell(get_options())[0]
    ):
...
def actions_for_url(url: str, actions_spec: str | None = None, mouse_event_mod: int = 0) -> Iterator[KeyAction]:
